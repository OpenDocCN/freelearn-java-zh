...
`sun.misc.Unsafe`类在 Java 9 中被标记为删除。实际上，在编程行业中对此决策有一些反对意见。为了平息他们的担忧，该类已被弃用，但不会完全删除。可以向 JVM 发送一个特殊标志来利用原始 API。
...
1.  `SamplePackage.Thing` 的解析开始。此步骤包括将 `SamplePackage.Thing` 的所有成员类型导入作用域。
