...
如前文代码所述，`window.performance`对象为我们提供了在`Browser Window 对象`内可用的性能指标。我们需要使用 JavaScript 来检索这个指标。在这里，我们正在收集`loadEventEnd`时间和`navigationEventStart`时间，并计算它们之间的差异，这将给我们页面加载时间。
